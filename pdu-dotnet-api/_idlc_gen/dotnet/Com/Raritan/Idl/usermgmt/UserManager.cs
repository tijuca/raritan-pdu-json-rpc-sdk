...
namespace Com.Raritan.Idl.usermgmt {
  public class UserManager : ObjectProxy {

    static public readonly new TypeInfo typeInfo = new TypeInfo("usermgmt.UserManager:1.0.0", null);

    public UserManager(Agent agent, string rid, TypeInfo ti) : base(agent, rid, ti) {}
    public UserManager(Agent agent, string rid) : this(agent, rid, typeInfo) {}

    public static new UserManager StaticCast(ObjectProxy proxy) {
      return proxy == null ? null : new UserManager(proxy.Agent, proxy.Rid, proxy.StaticTypeInfo);
    }
