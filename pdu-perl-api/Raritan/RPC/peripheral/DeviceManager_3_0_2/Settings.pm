...
package Raritan::RPC::peripheral::DeviceManager_3_0_2::Settings;
...
sub encode {
    ...
    foreach my $key0 (keys %{$in->{'defaultThresholdsMap'}}) {
        ...
        my $elem0 = {};
        $elem0->{'key'} = "$key0";
        $elem0->{'value'} = Raritan::RPC::sensors::NumericSensor_4_0_3::Thresholds::encode($value0);
        push(@{$encoded->{'defaultThresholdsMap'}}, $elem0);
    }
    $encoded->{'maxActivePoweredDryContacts'} = 1 * $in->{'maxActivePoweredDryContacts'};
    return $encoded;
}
...
sub decode {
    ...
    for (my $i0 = 0; $i0 <= $#{$in->{'defaultThresholdsMap'}}; $i0++) {
        ...
        my $value0 = Raritan::RPC::sensors::NumericSensor_4_0_3::Thresholds::decode($agent, $in->{'defaultThresholdsMap'}->[$i0]->{'value'});
        $decoded->{'defaultThresholdsMap'}->{$key0} = $value0;
    }
    $decoded->{'maxActivePoweredDryContacts'} = $in->{'maxActivePoweredDryContacts'};
    return $decoded;
}
